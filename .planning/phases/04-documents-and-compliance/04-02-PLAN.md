...
<task type="auto">
  ...
  <action>
**IMPORTANT PRE-FLIGHT:** Run the following before starting:
1. `pnpm --filter database exec prisma generate` — ensures the Prisma client has the Document model accessor.
2. `pnpm --filter api add -D @types/multer` — installs Multer type definitions required for `Express.Multer.File[]`. The API tsconfig has `noImplicitAny: true`, so file parameters MUST be typed; without this package TypeScript will error on `Express.Multer.File`.
...
- For `uploadMany`, upload each file using `supabaseAdmin.storage.from('documents').upload(path, file.buffer, { contentType: file.mimetype })`. The `file.buffer` is a Buffer because memoryStorage is used in the controller (see controller note below).
...
- `POST /` -- Upload. Use:
  ...
  ```
  Accept `@UploadedFiles() files: Express.Multer.File[]`, `@Body() body: UploadDocumentDto`, `@Param('clientId') clientId`, `@TenantId() tenantId`, `@CurrentUser() user`. Using `multer.memoryStorage()` is critical: it stores files in `file.buffer` (a Buffer in memory) rather than writing to disk. The service reads `file.buffer` to upload to Supabase Storage. Without this option the default disk storage would leave `file.buffer` undefined.
...
Confirm documents.controller.ts uses `multer.memoryStorage()` in the FilesInterceptor options and types files parameter as `Express.Multer.File[]`.
  </verify>
  <done>
Documents module fully implemented with upload (multi-file, Supabase Storage via memoryStorage so file.buffer is available), list (with folder grouping), signed URL (300s), delete (storage + Prisma + activity log), and counts endpoints. TimelineService extended with optional policyId parameter. All endpoints use JwtAuthGuard + RolesGuard with tenant isolation. API builds successfully.
  </done>
</task>
...
14. FilesInterceptor uses multer.memoryStorage() so file.buffer is available for Supabase Storage upload
15. @types/multer installed so Express.Multer.File[] type is available
</verification>
