...
<task type="auto">
  <name>Task 2: Client profile tabs (Documents + Compliance) + policy docs + count badges</name>
  <files>
    apps/web/src/components/clients/client-documents-tab.tsx
    apps/web/src/components/clients/client-compliance-tab.tsx
    apps/web/src/app/(dashboard)/clients/[id]/page.tsx
...
The user decision says "Document count badges visible on client list rows and policy cards."

Modify the client list to include document counts:

- **`apps/web/src/components/clients/client-list.tsx`**: If this component fetches clients, the backend should include document counts. Alternatively, add a `_count.documents` field to the client fetch. Check how the existing `_count.policies` is handled and follow the same pattern. If the backend already includes `_count`, just add `documents` to the select. If not, the clients backend endpoint needs to include `_count: { select: { documents: true } }` — check if this is feasible from the frontend side or if a backend change is needed. If a backend change is needed, update `apps/api/src/clients/clients.service.ts` to include `_count: { select: { ..., documents: true } }` in the findAll query.

- **`apps/web/src/components/clients/client-table.tsx`**: Add a "Docs" column showing the document count as a badge number. Use a small `FileText` icon with count.

- **`apps/web/src/components/clients/client-cards.tsx`**: Add document count to the card metadata (next to policies count if it exists).

**5. Add document count badges to policy cards:**

- **`apps/web/src/components/policies/policy-cards.tsx`**: Add a document count indicator to each policy card. This requires the policies API to include `_count: { select: { documents: true } }`. Check the policies backend — if not already including document counts, add it to the findAll query in `apps/api/src/policies/policies.service.ts`.

For both client and policy count badges: if the backend doesn't already return `_count.documents`, update the respective service's findAll/findOne methods to include `_count: { select: { documents: true } }` alongside existing count fields.

Use a small, subtle badge: `<span className="flex items-center gap-1 text-xs text-muted-foreground"><FileText className="size-3" /> {count}</span>`
  </action>
  <verify>
...
Verify client list shows document count badges.
...
9. Client list shows document count badges
10. Policy cards show document count badges
