...
Build issues fixed this session (2026-02-21):
- Ran `pnpm --filter database exec prisma generate` to regenerate Prisma client (27 TS errors resolved)
- Ran `pnpm --filter web add @tanstack/react-table date-fns` to install missing frontend deps (9 build errors resolved)
- Committed lockfile fix: 8b28abd
- User is testing on a REMOTE machine — needs to `git pull`, `pnpm install`, `prisma generate` before testing
...
<next_action>
On remote machine after git pull:
1. `pnpm install`
2. `pnpm --filter database exec prisma generate`
3. `pnpm dev` (starts both API + web)
4. Test at http://localhost:3000/clients (7 test scenarios in remaining_work)

Then resume with `/gsd:execute-phase 2` which will:
...
OR manually:
1. User tests at http://localhost:3000/clients
