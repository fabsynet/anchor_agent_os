...
must_haves:
  truths:
    ...
    - "Agent sees restricted nav items greyed out with 'Admin only' tooltip (Expenses, Settings/Team)"
...
<task type="auto">
  ...
  <done>
    Navy color theme applied. User hook fetches profile with role. Mode toggle switches themes. User menu provides logout. Nav items render with role-based visibility (disabled + tooltip for admin-only items when user is Agent).
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
    ...
    - Role-based nav (admin-only items greyed out for agents)
  ...
  <how-to-verify>
    ...
    8. (If possible) Test agent role visibility:
       - If you have an agent user: verify Expenses and Settings/Team items are greyed out with "Admin only" tooltip
  </how-to-verify>
...
3. Admin-only items are disabled with tooltip for Agent role
...
- AUTH-08: Agent sees restricted navigation (budget/expense disabled with "Admin only" tooltip)
