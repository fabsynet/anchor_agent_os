...
<task type="auto">
  ...
  <action>
    ...
    3. Create signup form component (`apps/web/src/components/auth/signup-form.tsx`):
       ...
       - On submit: call `supabase.auth.signUp()` with email, password, and options.data containing firstName, lastName, agencyName. Set emailRedirectTo to `${window.location.origin}/verify`
...
<task type="auto">
  <name>Task 2: Create verification, password reset, and update password pages</name>
  <files>
    apps/web/src/app/(auth)/verify/page.tsx
    apps/web/src/app/(auth)/reset-password/page.tsx
    apps/web/src/app/(auth)/update-password/page.tsx
  ...
  <action>
    ...
    2. Create auth callback route (`apps/web/src/app/auth/callback/route.ts`):
       ...
       - Determine redirect: if the user just verified their email (new signup), redirect to /setup. If password reset, redirect to /update-password. Use the `next` param or default to /
       - On error: redirect to /login with error param
       - IMPORTANT: This route handles ALL Supabase auth redirects (verification, password reset, invitation acceptance)
  ...
  <done>
    Email verification flow: signup -> verify page -> email link -> callback route -> session created -> redirect to app. Password reset flow: reset page -> email link -> callback route -> update password page -> new password saved. All pages use the split-screen auth layout.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    8. (Optional) If Supabase is configured and migration run: test actual signup, check email, verify, login
  </how-to-verify>
...
5. Session persists across browser refresh (middleware refreshes tokens via getUser)
6. Navigation between auth pages works (login <-> signup, login -> reset)
</verification>

<success_criteria>
- AUTH-01: Signup form with email/password + metadata fields, calls Supabase signUp
- AUTH-02: Verify page + auth callback route handle email verification flow
