...
must_haves:
  truths:
    ...
    - "Database trigger detects invited users via invitation_id in metadata and links them to existing tenant"
  ...
  key_links:
    ...
    - from: "packages/database/prisma/migrations"
      ...
      via: "handle_new_user trigger checks raw_user_meta_data->>'invitation_id'"
...
<task type="auto">
  ...
  <action>
    ...
    4. Create shared packages:
       - `packages/shared/`: TypeScript package with types, constants, and Zod validation schemas
         ...
         - `src/constants/roles.ts`: Export ROLES enum (ADMIN = 'admin', AGENT = 'agent'), PERMISSIONS map defining which nav items each role can access. Admin: all. Agent: dashboard, clients, policies, tasks, documents, expenses, notes (no settings/team). Define NAV_ITEMS array with id, label, href, icon name, adminOnly boolean. IMPORTANT per user decision: Expenses is NOT adminOnly — agents CAN submit expenses. Only Settings/Team should be adminOnly. The NAV_ITEMS should be:
           - Dashboard (adminOnly: false)
           - Clients (adminOnly: false)
           - Policies (adminOnly: false)
           - Tasks (adminOnly: false)
           - Documents (adminOnly: false)
           - Expenses (adminOnly: false) -- agents can submit expenses per locked user decision
           - Settings (adminOnly: true) -- only the team management sub-page is admin-restricted
  ...
  <done>
    Monorepo runs. Both apps start. Shared types and Zod schemas compile and export. All directories exist per the architecture from RESEARCH.md. NAV_ITEMS correctly marks only Settings as adminOnly — Expenses is accessible to all roles per user decision.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    4. Create a SQL migration file (manually, not via prisma migrate) at `packages/database/prisma/migrations/00000000000000_init/migration.sql` that includes:
       ...
       IMPORTANT — Invited user handling in the trigger:
       The `handle_new_user()` function MUST handle BOTH organic signups AND invited users. The detection mechanism works as follows:

       a. When admin invites via Supabase `inviteUserByEmail()`, the `data` option passes metadata:
          `{ invitation_id: '<uuid>', tenant_id: '<uuid>', role: 'admin' | 'agent' }`
          This metadata is stored in `auth.users.raw_user_meta_data` when Supabase creates the auth user.

       b. The trigger function checks `NEW.raw_user_meta_data->>'invitation_id'`:
          - IF invitation_id IS NOT NULL (invited user):
            1. Extract `v_invitation_id := NEW.raw_user_meta_data->>'invitation_id'`
            2. Extract `v_tenant_id := (NEW.raw_user_meta_data->>'tenant_id')::uuid`
            3. Extract `v_role := NEW.raw_user_meta_data->>'role'`
            4. Verify the invitation exists and is pending: `SELECT id FROM public.invitations WHERE id = v_invitation_id AND status = 'pending'`
            5. If valid: Create user profile with the extracted tenant_id and role (NOT creating a new tenant)
            6. Update the invitation status to 'accepted': `UPDATE public.invitations SET status = 'accepted' WHERE id = v_invitation_id`
          - ELSE (organic signup — has agency_name in metadata):
            1. Create a new tenant from `NEW.raw_user_meta_data->>'agency_name'`
            2. Generate slug
            3. Create user profile with role 'admin' linked to the new tenant

       c. Add verification: The trigger should handle edge cases (missing metadata gracefully, log errors if invitation not found).
  ...
  <verify>
    ...
    - Migration SQL contains logic to check `raw_user_meta_data->>'invitation_id'` for invited user detection
  ...
  <done>
    Prisma schema defines all Phase 1 models. Migration SQL includes table creation + trigger functions. The handle_new_user trigger correctly handles both organic signups (creates new tenant) and invited users (looks up invitation by invitation_id from raw_user_meta_data, links to existing tenant with correct role, marks invitation accepted). Supabase client utilities are ready for browser and server use. Middleware handles session refresh and route protection. API client utility is ready for NestJS communication.
  </done>
</task>
...
9. NAV_ITEMS has Expenses as adminOnly: false (accessible to all roles)
</verification>
...
- SQL migration includes handle_new_user trigger that handles both organic signups AND invited users (checking raw_user_meta_data->>'invitation_id')
- SQL migration includes custom_access_token_hook
...
- NAV_ITEMS marks only Settings/Team as adminOnly — Expenses is accessible to all roles per user decision
</success_criteria>
